...
packs (backend pinned `>=2.9.0`). Each has a committed `retread-*.lock.json` capturing its
resolved closure.
...
full cold resolve. Without the env var (the default), every install is a full resolve — identical
to before. Optionally set `RETREAD_VERIFY_LOCALADD=1` to log an internal-consistency check of the
resulting closure.
